...
function TypingIndicator() {
  return (
    <div className="flex items-center gap-3 py-0.5" aria-label="AI is responding">
      <div className="flex items-center gap-1.5" aria-hidden>
        {[0, 1, 2].map(i => (
          <span
            key={i}
            className="typing-dot w-1.5 h-1.5 rounded-full bg-accent"
            style={{ animationDelay: `${i * 0.16}s` }}
          />
        ))}
      </div>
      <span className="text-xs text-ink-faint font-medium">Thinking…</span>
    </div>
  );
}
...
function StreamingCursor() {
  return (
    <span
      className="streaming-cursor inline-block w-0.5 h-[1.05em] ml-0.5 align-text-bottom bg-accent rounded-full"
      aria-hidden
    />
  );
}
...
export function MessageBubble({ role, content, sources, streaming }: Props) {
  ...
  const isThinking = !isUser && streaming && content.length === 0;
    ...
    >
      <div
        className={`
          ...
          ${isThinking ? 'ring-2 ring-accent/25 ring-offset-1 ring-offset-bg-elevated' : ''}
        `}
      >
        {isUser ? 'U' : 'AI'}
      </div>
      ...
      <div className={`flex-1 min-w-0 max-w-[85%] ${isUser ? 'items-end' : 'items-start'} flex flex-col gap-2`}>
        <div
          className={`
            px-4 py-3 rounded-2xl text-sm leading-relaxed transition-colors duration-300
            ${isUser
              ? 'bg-accent text-white rounded-tr-sm shadow-sm'
              : `bg-bg-elevated border rounded-tl-sm text-ink ${
                  isThinking ? 'border-accent/25 shadow-sm shadow-accent/5' : 'border-bg-border'
                }`
            }
          `}
        >
